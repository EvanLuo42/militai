...
pub fn build(b: *std.Build) void {
    ...
    const common_mod = b.addModule("common", .{ .root_source_file = b.path("common/bootinfo.zig") });
    ...
    bootloader.root_module.addImport("common", common_mod);
    ...
    kernel.root_module.addImport("common", common_mod);
    ...
    const img_path = "os.img";

    const make_img = b.addSystemCommand(&.{ "dd", "if=/dev/zero", "of=" ++ img_path, "bs=1M", "count=64" });

    const fmt_img = b.addSystemCommand(&.{ "mkfs.vfat", "-F", "32", "-n", "MYOS_EFI", img_path });

    const mkdir_dirs = b.addSystemCommand(&.{ "mmd", "-i", img_path, "::/EFI", "::/EFI/BOOT" });

    const boot_src = b.getInstallPath(.{ .custom = "efi/boot" }, "bootx64.efi");
    const copy_boot = b.addSystemCommand(&.{ "mcopy", "-i", img_path, "-o", boot_src, "::/EFI/BOOT/BOOTX64.EFI" });

    const kernel_src = b.getInstallPath(.{ .custom = "" }, "kernel.bin");
    const copy_kernel = b.addSystemCommand(&.{ "mcopy", "-i", img_path, "-o", kernel_src, "::/kernel.bin" });
    ...
    const run_cmd = b.addSystemCommand(&.{ "qemu-system-x86_64", "-m", "1G", "-bios", "/usr/share/OVMF/OVMF_CODE.fd", "-drive", "format=raw,file=" ++ img_path, "-serial", "stdio" });
